(function(app)
{
    ...
    {
        /* var dropDownID = 1;   
        var imageThumbHolderID = 1;
        var imageThumbHolderID2 = 1;  

        $('.imageBoxHolder').each(function(){
            if($(this).hasClass("hs_set2")){
                var imageBoxHolderID = String($(this).attr("class").split(" ")[1]).slice(-1);
                console.log("hs_set2", imageThumbHolderID);
                var imageThumbHolderDiv = "<div class='imageThumbHolder imageThumbHolder_"+imageThumbHolderID+"' id='imageThumbHolder_"+imageThumbHolderID+"' data-imageBoxHolderid='"+imageBoxHolderID+"'></div>" 
                $(imageThumbHolderDiv).insertBefore(this);
                imageThumbHolderID++;
            }                          
        });   */        
        
        /* $('.midDiv').each(function(){
           $(this).find(".imageBoxHolder").each(function(index){ 
                var imageBoxHolderID = String($(this).attr("class").split(" ")[1]).slice(-1);            
                var imageSrc = $(this).find(".graphContainer img:first-child").attr("src");
                var imageAlt = $(this).find(".graphContainer img:first-child").attr("alt")
                // console.log("imageBoxHolder", index, imageBoxHolderID, imageSrc, imageAlt);
                var thumbnail = '<span class="thumbnail" role="button" tabindex="0" data-imageBoxHolderid="'+imageBoxHolderID+'" data-dropdownid="'+dropDownID+'" data-id="0" aria-label="'+imageAlt+'" ><img src="'+imageSrc+'" class="thumbImg" alt="'+imageAlt+'" /></span>';                 
                console.log("hs_set1", imageThumbHolderID2);
                $("#imageThumbHolder_"+imageThumbHolderID2).append(thumbnail); 
            })  
            $(this).find(".hs_set1").each(function(index){
                //console.log("hs_set1", this);
                dropDownID++;
                imageThumbHolderID2++;
            })
        }); */
    ...
    function onThumbnailSelect(e)
    {
        ...
        var imageBoxHolderId = $(e.currentTarget).attr("data-imageBoxHolderid");
    ...
    {
        // console.log("fnCheckNextBack", nSlideCounter)
        ...
        mainDiv.find(".hs_set1 .dropdownList li").removeClass("selected");       
        $(mainDiv.find(".hs_set1 .dropdownList li")[0]).addClass("selected");
        mainDiv.find(".hs_set1 .dropdownList").find('.current').text($(mainDiv.find(".hs_set1 .dropdownList li")[0]).text());   

        $('.AnswerDiv').text('Show answer');
        $('.AnswerDiv').attr("aria-pressed","false");
        var AnswerDivImageBoxHolderId = mainDiv.find(".AnswerDiv").attr("data-imageBoxHolderid");
        $('.imageBoxHolder' + AnswerDivImageBoxHolderId).hide();
        $('.imageThumbHolder' + AnswerDivImageBoxHolderId).hide();
        // console.log("AnswerDivImageBoxHolderId ",'.imageThumbHolder' + AnswerDivImageBoxHolderId)
        $('.textArea').hide();
        if (nSlideCounter < 0)
        {
            $(".topContent").hide();
    ...
    ActivityMain.setHabitatContainerSize = function(){
        ...
        //console.log(midConatinerHeight);
